...
def main():
    ...
    parser.add_argument(
        ...
        dest="s_names",
        default=[],
        help="Sample names to use as headers in the new report",
    )
    parser.add_argument(
        "--only-combined",
        required=False,
    ...
    if len(sample_names) > 0 and len(sample_names) != num_samples:
        sys.stderr.write(
            "Number of sample names provided does not match number of reports\n"
        )
        sys.exit(1)
    # Map names
    id2names = {}
    ...
    while len(all_nodes) > 0:
        ...
        o_file.write(
            "%0.4f\t" % (float(curr_node.tot_all) / float(total_reads[0]) * 100)
        )
