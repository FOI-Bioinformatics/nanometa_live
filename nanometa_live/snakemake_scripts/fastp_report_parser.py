...
with open(in_file, "r") as inF:
    ...
    passed_filter_reads = filter_data.get("filtering_result", {}).get(
        "passed_filter_reads", 0
    )
    low_quality_reads = filter_data.get("filtering_result", {}).get(
        "low_quality_reads", 0
    )
    too_many_N_reads = filter_data.get("filtering_result", {}).get(
        "too_many_N_reads", 0
    )
...
with open(out_file, "w") as outF:
    csv_row = (
        str(passed_filter_reads)
        + ","
        + str(low_quality_reads)
        + ","
        + str(too_many_N_reads)
        + ","
        + str(too_short_reads)
        + "\n"
    )
