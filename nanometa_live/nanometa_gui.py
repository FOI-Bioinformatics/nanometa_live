...
from nanometa_live.helpers.gui_scripts import (
    get_time,
    sankey_placeholder,
    kreport2_df,
    tax_hierarchy_list,
...
def create_sankey_data(selected_domains, clade_list, top_filter=5):
    ...
    if update_disabled == False:  # if True: raw_df is not updated
        while not os.path.isfile(
            kreport_file
        ):  # if there are no data files yet
    ...
    top_df, ghost_nodes = filter_by_top(
        top_filter, edges_df, result_matrix, tax_letters, rev_tax_letters
    )
...
config_file_path = (
    os.path.join(args.path, args.config) if args.path else args.config
)
...
kreport_file = os.path.join(
    config_contents["main_dir"], "kraken_cumul/kraken_cumul_report.kreport2"
)
...
fastp_file = os.path.join(
    config_contents["main_dir"], "fastp_reports/compiled_fastp.txt"
)
...
update_toggle = daq.ToggleSwitch(
    id="update_toggle",
    label="Live updating: ",
...
update_toggle_tooltip = dbc.Tooltip(
    "Toggle real-time interface updates on/off. \
...
update_status = html.Div(id="update_status", style={"textAlign": "center"})

# Displays the time when the last update happened.
timestamp = html.Div(
    id="timestamp", style={"textAlign": "center"}, children=time_token
)

# Change update frequency
update_frequency_box = html.Div(
    children=[
        html.Label(
            "Update frequency (seconds):", style={"padding-right": "10px"}
        ),
        html.Br(),
        dcc.Input(
            id="update_frequency",
...
update_frequency_tooltip = dbc.Tooltip(
    "The number of seconds between GUI updates.",
    ...
    [
        ...
        dbc.Modal(
            [
                ...
                dbc.ModalBody(
                    "Are you sure you want to shut down the program?"
                ),
                dbc.ModalFooter(
                    [
                        dbc.Button(
                            "No", id="confirm-no-button", color="secondary"
                        ),
                        dbc.Button(
                            "Yes", id="confirm-yes-button", color="danger"
                        ),
                    ]
                ),
            ],
                ...
                dbc.ModalBody(
                    [
                        dcc.Input(
                            id="filename-input-1",
                            type="text",
                            placeholder="Enter a file name or path",
                        ),
                        html.Div(
                            'The file will be saved as a ".kreport2" file.'
                        ),
                        html.Div(
                            'If only a file name is specified, the report will be saved in "PROJECT_DIRECTORY/reports/" by default.'
                        ),
                    ]
                ),
                dbc.ModalFooter(
...
upper_gui_layout = html.Div(
    [
        html.Div(
            dbc.Container([main_title, subtext]),
            style={"margin-right": "100px"},
        ),
        html.Div(
            dbc.Container(
                [update_toggle, update_toggle_tooltip, update_status, timestamp]
            ),
    ...
    [
        ...
        dbc.Modal(
            [
                ...
                dbc.ModalFooter(
                    dbc.Button(
                        "Close", id="sankey-close-button", className="ml-auto"
                    )
                ),
            ],
            id="sankey-modal",
...
filter_headline = html.Label(
    "Filter by top reads at each taxonomic level:",
...
sankey_top_tooltip = dbc.Tooltip(
    "For example, if set to 5, the top 5 taxa with the highest reads at each level will be included.",
    target="filter_value",
    placement="top",
    delay={"show": 1000},
)
...
choose_domains = html.Div(
    children=[
        html.Label("Domains to include:", style={"padding-right": "10px"}),
        dcc.Checklist(
            ["Bacteria", "Archaea", "Eukaryota", "Viruses"],
            ["Bacteria", "Archaea", "Eukaryota", "Viruses"],
            id="domains",
...
choose_hierarchy = html.Div(
    children=[
        html.Label(
            "Taxonomic levels to include:", style={"padding-right": "10px"}
        ),
        dcc.Checklist(
...
filter_submit = html.Button(id="filter_submit", n_clicks=0, children="Filter")

# Tooltip for Sankey submit button.
sankey_button_tooltip = dbc.Tooltip(
    "Apply your filters. Filters will also be applied automatically at each update.",
...
sankey_filtering = html.Div(
    [
        ...
        html.Div(
            [filter_headline, filter_input, sankey_top_tooltip],
            className="bg-light border",
        ),
        html.Div(choose_domains, className="bg-light border"),
        html.Div(choose_hierarchy, className="bg-light border"),
        html.Div(
            [filter_submit, sankey_button_tooltip], className="bg-light border"
        ),
    ],
    className="hstack gap-3",
)
...
# Colored table.
pathogen_table = dbc.Container(
    [
        dbc.Label("Species of interest:"),
...
blast_info = html.Div(
    [
        ...
        + str(config_contents["min_perc_identity"]),
        html.Br(),
...
validate_option = html.Div(
    children=[
        html.Label("BLAST validation"),
        dcc.Checklist(["Validate"], id="validate_box"),
    ],
    className="bg-light border",
)

# Tooltip for validation checkbox.
validation_tooltip = dbc.Tooltip(
    "Adds an additional column with the number of reads validated by BLAST, \
                                 using a minimum percent identity of "
    + str(config_contents["min_perc_identity"])
    + " and\
                                 an e-value cutoff of "
...
# color threshold
color_threshold = html.Div(
    children=[
        html.Label("Coloring threshold:", style={"padding-right": "10px"}),
        html.Br(),
        dcc.Input(id="threshold_val", value="100", type="number"),
    ],
    className="bg-light border",
)

danger_threshold_tooltip = dbc.Tooltip(
    "Species of interest with a number of reads above this threshold will be \
...
# Pahogen barchart.
pathogen_fig_obj = dcc.Graph(id="pathogen_fig", figure=pathogen_fig)

# pathogen filter button
filter_pathogen = html.Button(
    id="filter_pathogen", n_clicks=0, children="Apply"
)

# pathogen filter button tooltip
filter_pathogen_tooltip = dbc.Tooltip(
    "Apply your changes",
...
pathogen_info_line3 = (
    'The "Tax ID" column contains the taxonomic IDs from the databased used.'
)
...
pathogen_info_line6 = "Minimum percent identity: " + str(
    config_contents["min_perc_identity"]
)
    ...
    [
        ...
        dbc.Modal(
            [
                ...
                dbc.ModalFooter(
                    dbc.Button(
                        "Close", id="pathogen-close-button", className="ml-auto"
                    )
                ),
            ],
            id="pathogen-modal",
            ...
            [
                ...
                dbc.ModalBody(
                    [
                        ...
                        html.Div(
                            'If only a file name is specified, the list will be saved in "PROJECT_DIRECTORY/reports/" by default.'
                        ),
                    ]
                ),
                dbc.ModalFooter(
...
pathogen_buttons = html.Div(
    [filter_pathogen, pathogen_modal, export_pathogens_button],
    className="hstack gap-3",
)
...
top_list = dbc.Container(
    [
        dbc.Label(
            "This section provides a quick overview of the most abundant hits, offering you an immediate glimpse into the microbial diversity and prevalence."
        ),
        create_top_table(),
    ]
)
...
toplist_filter_head = html.Label(
    "Number of taxa to include:", style={"padding-right": "10px"}
)  # headline

top_filter_val = dcc.Input(
    id="top_filter_val", value="20", type="number"  # filter value
)

# Tooltip for top list filtering.
top_list_tooltip = dbc.Tooltip(
    "The number of entries to include in the list, i.e. the lenght of the list.",
...
toplist_submit = html.Button(id="toplist_submit", n_clicks=0, children="Filter")

# Tooltip for toplist filter submit button.
toplist_button_tooltip = dbc.Tooltip(
    "Apply your filters. Filters will also be applied automatically at each update.",
...
toplist_info_line2 = (
    'The column "Tax ID" contains the taxonomic IDs from the database used.'
)
    ...
    [
        ...
        dbc.Modal(
            [
                ...
                dbc.ModalFooter(
                    dbc.Button(
                        "Close", id="toplist-close-button", className="ml-auto"
                    )
                ),
            ],
            id="toplist-modal",
...
toplist_filtering = html.Div(
    [
        html.Br(),
        html.Div(
            [toplist_filter_head, top_filter_val, top_list_tooltip],
            className="bg-light border",
        ),
...
toplist_col_1 = html.Div([toplist_head, top_list], className="bg-light border")
...
qc_filtering_headline = html.Div(
    "FILTERING", style={"padding-right": "10px"}, id="qc_filtering_headline"
)
qc_reads_pre_filtering = html.Div(
    "Total reads pre filtering:",
    style={"padding-right": "10px"},
    id="qc_reads_pre_filtering",
)

qc_reads_passed = html.Div(
    "Reads that passed filtering:",
    style={"padding-right": "10px"},
    id="qc_reads_passed",
)
qc_reads_removed = html.Div(
    "Total reads removed:",
    style={"padding-right": "10px"},
    id="qc_reads_removed",
)

qc_remove_reason = html.Div(
    "REASONS FOR REMOVAL",
    style={"padding-right": "10px"},
    id="qc_remove_reason",
)
qc_proportions_info = html.Div(
    "(percentages of total removed reads)",
    style={"padding-right": "10px"},
    id="qc_proportions_info",
)
qc_low_quality = html.Div(
    "Too low quality:", style={"padding-right": "10px"}, id="qc_low_quality"
)
qc_too_short = html.Div(
    "Too short:", style={"padding-right": "10px"}, id="qc_too_short"
)
qc_low_complexity = html.Div(
    "Too low complexity:",
    style={"padding-right": "10px"},
    id="qc_low_complexity",
)

qc_classification_headline = html.Div(
    "CLASSIFICATION",
    style={"padding-right": "10px"},
    id="qc_classification_headline",
)
qc_classified_reads = html.Div(
    "Classified reads:",
    style={"padding-right": "10px"},
    id="qc_classified_reads",
)
qc_unclassified_reads = html.Div(
    "Unclassified reads:",
    style={"padding-right": "10px"},
    id="qc_unclassified_reads",
)

qc_processing_headline = html.Div(
    "FILE PROCESSING",
    style={"padding-right": "10px"},
    id="qc_processing_headline",
)
waiting_files = html.Div(
    "Files awaiting processing:",
    style={"padding-right": "10px"},
    id="waiting_files",
)
processed_files = html.Div(
    "Files processed:", style={"padding-right": "10px"}, id="processed_files"
)
    ...
    [
        ...
        dbc.Modal(
            [
                ...
                dbc.ModalFooter(
                    dbc.Button(
                        "Close", id="qc-close-button", className="ml-auto"
                    )
                ),
            ],
            id="qc-modal",
...
qc_row_1 = html.Div(
    [
        html.Div(
            dcc.Graph(id="cumul_reads_graph", figure=cumul_reads_fig),
            className="bg-light border",
        ),
        html.Div(
            dcc.Graph(id="cumul_bp_graph", figure=cumul_bp_fig),
            className="bg-light border",
        ),
    ],
    className="hstack gap-3",
...
qc_row_2 = html.Div(
    [
        html.Div(
            dcc.Graph(id="reads_graph", figure=reads_fig),
            className="bg-light border",
        ),
        html.Div(
            dcc.Graph(id="bp_graph", figure=bp_fig), className="bg-light border"
        ),
    ],
    className="hstack gap-3",
)
...
qc_column = html.Div([qc_row_1, qc_row_2], className="vstack gap-3")
...
qc_layout = html.Div([html.Br(), qc_head, html.Br(), qc_row_all])
...
sunburst_chart = dcc.Graph(id="sunburst_chart", figure=sunburst_fig)

# Sunburst filtering.
sun_filter_head = html.Label(
    "Filter by minimum reads:", style={"padding-right": "10px"}
)

sun_filter_val = dcc.Input(id="sun_filter_val", value="10", type="number")

# Tooltip for sunburst filtering.
sunburst_filter_tooltip = dbc.Tooltip(
    "Include only taxa with at least this many reads.",
...
sun_submit = html.Button(id="sun_submit", n_clicks=0, children="Filter")

# Tooltip for sunburst submit button.
sunburst_button_tooltip = dbc.Tooltip(
    "Apply your filters. Filters will also be applied automatically at each update.",
    target="sun_submit",
    placement="top",
    delay={"show": 1000},
    ...
    [
        ...
        dbc.Modal(
            [
                ...
                dbc.ModalFooter(
                    dbc.Button(
                        "Close", id="sunburst-close-button", className="ml-auto"
                    )
                ),
            ],
            id="sunburst-modal",
...
sun_filtering = html.Div(
    [
        html.Div(
            [sun_filter_head, sun_filter_val, sunburst_filter_tooltip],
            className="bg-light border",
        ),
...
sunburst_complete = html.Div(
    [
        html.Div(sunburst_chart, className="bg-light border"),
    ...
    [
        dcc.Tabs(
            [
                dcc.Tab(
                    label="Main",
                    children=[pathogens_top_with_margin, html.Br()],
                ),
                dcc.Tab(
                    label="QC",
                    children=[qc_with_margin, html.Br(), html.Br(), html.Br()],
                ),
                dcc.Tab(
                    label="Sankey plot",
                    children=[
...
app.layout = html.Div(
    [upper_gui_layout, html.Br(), main_tabs, interval_component]
)
...
@app.callback(
    Output("timestamp", "children"),  # plain text
...
@app.callback(
    Output("update_status", "children"),  # text info on state
    Output("interval_component", "disabled"),  # actual on/off
    Input("update_toggle", "value"),
)  # toggle is clicked: bool
...
@app.callback(
    Output("interval_component", "interval"), Input("update_frequency", "value")
)
...
@app.callback(
    ...
    State("domains", "value"),
    State("clades", "value"),
)  # all the filters are states until click
def update_sankey(
    interval_trigger, filter_click, filter_value, domains, clades
):
    global sankey_fig
    ...
    sankey_fig = go.Figure(
        create_sankey_data(domains, fixed_clades, int(filter_value))
    )
    sankey_fig_layout()
    return sankey_fig
...
@app.callback(
    ...
    State("sun_domains", "value"),
)  # all the filters are states until click
def update_sunburst(interval_trigger, filter_click, filter_value, domains):
    data = icicle_sunburst_data(
        raw_df, domains, int(filter_value), config_file_path=config_file_path
    )
...
@app.callback(
    Output("pathogen_fig", "figure"),  # barchart
...
def pathogen_update(interval_trigger, click, val_state, threshold):
    ...
    species_dict = {
        entry["taxid"]: entry["name"]
        for entry in config_contents["species_of_interest"]
    }
    ...
    for taxid in pathogen_list:
        if taxid not in pathogen_info["Tax ID"].values:
            ...
            pathogen_info.loc[len(pathogen_info.index)] = [
                species_name,  # add pathogen name
    ...
    pathogen_info["Color"] = pathogen_info["Reads"].apply(
        lambda x: "Green" if x < dll else "Red"
    )

    # Create the barchart from the pathogen_info table.
    pathogen_barchart_fig = px.bar(
        pathogen_info,
        x="Name",
        y="Reads",
        color="Color",
    ...
    # Change hover info.
    pathogen_barchart_fig.update_traces(
    ...
    if val_state:
        ...
        if len(val_state) == 1:
            ...
            validated_col = validation_col(
                validation_list, blast_dir, read_nr_list
            )
            # add to table
...
@app.callback(
    Output("top_table", "data"),  # row data for table
...
# If interval is disabled, it should keep the latest values.
@app.callback(
    Output("cumul_reads_graph", "figure"),  # plotly express plots
...
# If interval is disabled, it should keep the latest values.
@app.callback(
    Output("qc_classified_reads", "children"),  # text outputs
...
def update_qc_text(interval_trigger):
    ...
    if tot_reads_pre_filt == 0:
        ...
        percentage_passed_reads = float(
            round((tot_passed_reads * 100) / tot_reads_pre_filt, 1)
        )
        percentage_reads_removed = float(
            round((tot_removed_reads * 100) / tot_reads_pre_filt, 1)
        )
    ...
    if tot_removed_reads == 0:
        ...
        percentage_low_quality = float(
            round((tot_low_quality_reads * 100) / tot_removed_reads, 1)
        )
        percentage_low_complexity = float(
            round((tot_too_many_N_reads * 100) / tot_removed_reads, 1)
        )
        percentage_too_short = float(
            round((tot_too_short_reads * 100) / tot_removed_reads, 1)
        )

    # Create layout objects.
    trp = (
        "Total reads passed: "
        + str(tot_passed_reads)
        + " ("
        + str(percentage_passed_reads)
        + "%)"
    )
    tlq = (
        "Too low quality: ",
        str(tot_low_quality_reads) + " (" + str(percentage_low_quality) + "%)",
    )
    ts = (
        "Too short: ",
        str(tot_too_short_reads) + " (" + str(percentage_too_short) + "%)",
    )
    tlc = (
        "Too low complexity: ",
        str(tot_too_many_N_reads)
        + " ("
        + str(percentage_low_complexity)
        + "%)",
    )

    trr = (
        "Total reads removed: "
        + str(tot_removed_reads)
        + " ("
        + str(percentage_reads_removed)
        + "%)"
    )
...
@app.callback(
    Output("waiting_files", "children"),  # simple text output
    ...
    if button_id == "save-button-1":
        ...
        try:
            ...
            if os.path.isabs(filename):
                file_path = (
                    f"{filename}.kreport2"  # User specified an absolute path
                )
            else:
                file_path = os.path.join(
                    reports_directory, f"{filename}.kreport2"
                )  # User specified only a file name
    ...
    if button_id == "save-button-2":
        ...
        try:
            ...
            if os.path.isabs(filename):
                ...
                file_path = os.path.join(
                    reports_directory, f"{filename}.csv"
                )  # User specified only a file name
