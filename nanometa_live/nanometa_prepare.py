...
from nanometa_live.helpers.blast_utils import (
...
from nanometa_live.helpers.file_utils import (
    write_accessions_to_file,
    process_local_files,
    read_and_process_gtdb_metadata,
    download_gtdb_metadata,
    check_genome_files_existence,
)
from nanometa_live.helpers.kraken_utils import (
    run_kraken2_inspect,
    parse_kraken2_inspect,
    download_database,
    decompress_database,
)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Fetch and filter species data."
    )
    parser.add_argument(
        "-x",
        "--prefix",
        default="parsed_species_data",
        help="Prefix for the output CSV file.",
    )
    parser.add_argument(
        "--config",
        default="config.yaml",
    ...
    parser.add_argument(
        "--mode",
        default="gtdb-api",
        choices=["gtdb-api", "gtdb-file", "local-species", "local-taxid"],
    ...
    if external_db_key and external_db_key in external_db_info:
        ...
        db_extract_folder = os.path.join(
            kraken_db_folder, external_db_key
        ...
        update_config_file_with_comments(
            args.path, args.config, "kraken_taxonomy", external_kraken_taxonomy
        )

        if args.dry_run:
            logging.info(
                f"[DRY RUN] Would download Kraken2 database '{external_db_key}' from {db_url}"
            )
            sys.exit(0)

        db_file_name = os.path.join(
            kraken_db_folder, f"{external_db_key}.tar.gz"
        )
        ...
        if not os.path.exists(db_file_name):
            ...
            logging.info(
                f"Database file '{db_file_name}' already exists. Skipping download."
            )
        ...
        if not args.dry_run:
            ...
            if not os.path.exists(db_extract_folder) or not os.path.exists(
                hash_file_path
            ):
                if not os.path.exists(db_extract_folder):
                    os.makedirs(db_extract_folder)

                decompress_success = decompress_database(
                    db_file_name, db_extract_folder
                )
                ...
                logging.info(
                    f"Database '{external_db_key}' is already decompressed. Skipping decompression."
                )

    else:
        if external_db_key:
            logging.info(
                "External Kraken2 database key not set or invalid selection."
            )
        else:
    ...
    inspect_file_name = os.path.join(
        data_files_folder, generate_inspect_filename(kraken_db)
    )
    ...
    missing_genomefiles = check_genome_files_existence(
        data_files_folder, species_taxid_dict
    )
    print(species_list)
    ...
    if missing_genomefiles:
        ...
        if args.mode in ["gtdb-api"]:
            ...
            results = update_results_with_taxid_dict(
                results, species_taxid_dict
            )

            # Filter the results to include only exact matches and convert it to a DataFrame.
            filtered_results = filter_data_by_exact_match(
                results, kraken_taxonomy
            )
            ...
            output_file = os.path.join(
                data_files_folder, f"{args.prefix}_{kraken_taxonomy}.csv"
            )
            ...
            logging.info(
                f"Extracted assembly accessions for download: {accessions_to_download}"
            )
            ...
            if not os.path.exists(metadata_folder):
                ...
                logging.info(
                    f"Created metadata directory at {metadata_folder}."
                )

            gtdb_metadata_file = os.path.join(
                metadata_folder, "bac120_metadata.tsv.gz"
            )
            ...
            filtered_results = read_and_process_gtdb_metadata(
                gtdb_metadata_file, kraken_taxonomy, species_list
            )
            ...
            success = process_local_files(
                indata_folder, args.path, species_taxid_dict, id_type="species"
            )
            if not success:
            ...
            indata_folder = os.path.join(args.path, "indata")
            success = process_local_files(
                indata_folder, args.path, species_taxid_dict, id_type="taxid"
            )
            if not success:
        ...
        if args.mode in ["gtdb-api", "gtdb-file"]:
            ...
            accession_file = os.path.join(
                data_files_folder,
                f"{args.prefix}_{kraken_taxonomy}_accessions.txt",
            )
            ...
            decompress_and_rename_zip(
                f"{args.prefix}_ncbi_download.zip", df, data_files_folder
            )
