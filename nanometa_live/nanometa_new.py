...
def main():
    ...
    # Main config
    parser.add_argument(
        "-p", "--path", required=True, help="The path to the project directory."
    )
    parser.add_argument(
        "-c",
        "--config",
    ...
    # GUI Config
    parser.add_argument(
        "--analysis_name", type=str, help="Name of the analysis"
    )
    parser.add_argument(
        "--species_of_interest",
        type=str,
        help="Path to file containing species of interest",
    )
    parser.add_argument(
        "--danger_lower_limit",
        type=int,
        help="Danger cutoff for species abundance",
    )
    parser.add_argument(
        "--taxonomic_hierarchy_letters",
        type=str,
        nargs="*",
        help="Taxonomic hierarchy levels used by Kraken2",
    )
    parser.add_argument(
        "--default_hierarchy_letters",
        type=str,
        nargs="*",
        help="Default taxonomy levels displayed in the Sankey plot",
    )
    parser.add_argument(
        "--default_reads_per_level",
        type=int,
        help="Default number of entries per taxonomy level in the Sankey plot",
    )
    parser.add_argument(
        "--update_interval_seconds",
        type=int,
        help="GUI update frequency in seconds",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        help="Package management for Snakemake (None/conda)",
    )
    parser.add_argument(
        "--conda_frontend", type=str, help="Conda frontend to use (mamba/conda)"
    )
    parser.add_argument(
        "--nanopore_output_directory",
        type=str,
        help="Path to Nanopore output directory",
    )
    parser.add_argument(
        "--remove_temp_files",
        type=str,
        help="Whether to remove temporary files (yes/no)",
    )
    parser.add_argument(
        "--check_intervals_seconds",
        type=int,
        help="Workflow frequency in seconds",
    )
    parser.add_argument(
        "--snakemake_cores", type=int, help="Number of cores for Snakemake"
    )
    parser.add_argument(
        "--kraken_cores", type=int, help="Number of cores for Kraken2"
    )
    parser.add_argument(
        "--validation_cores", type=int, help="Number of cores for KrakenTools"
    )
    parser.add_argument(
        "--blast_cores", type=int, help="Number of cores for BLAST"
    )
    parser.add_argument(
        "--kraken_db", type=str, help="Path to Kraken2 database"
    )
    parser.add_argument(
        "--external_kraken2_db",
        type=str,
        help="Key for external Kraken2 database to use",
    )
    parser.add_argument(
        "--kraken_taxonomy",
        type=str,
        help="Type of taxonomy for Kraken (gtdb/ncbi)",
    )
    parser.add_argument(
        "--kraken_memory_mapping", type=str, help="Memory mapping for Kraken2"
    )
    parser.add_argument(
        "--blast_validation", type=bool, help="Turn BLAST validation on/off"
    )
    parser.add_argument(
        "--min_perc_identity",
        type=float,
    ...
    parser.add_argument(
        "--e_val_cutoff", type=float, help="E-value cutoff for BLAST"
    )
    ...
    if args.analysis_name:
        update_config_file_with_comments(
            args.path, args.config, "analysis_name", args.analysis_name
        )
    ...
    if args.species_of_interest:
        ...
        if species_list:
            update_species_of_interest(
                project_path, config_file_name, species_list
            )
    ...
    if args.gui_port:
        update_config_file_with_comments(
            args.path, args.config, "gui_port", args.gui_port
        )
    ...
    if args.conda_frontend:
        update_config_file_with_comments(
            args.path, args.config, "conda_frontend", args.conda_frontend
        )

    if args.nanopore_output_directory:
        nanopore_output_directory = os.path.abspath(
    ...
    if args.remove_temp_files:
        update_config_file_with_comments(
            args.path, args.config, "remove_temp_files", args.remove_temp_files
        )
    ...
    if args.snakemake_cores is not None:
        update_config_file_with_comments(
            args.path, args.config, "snakemake_cores", args.snakemake_cores
        )

    if args.kraken_cores is not None:
        update_config_file_with_comments(
            args.path, args.config, "kraken_cores", args.kraken_cores
        )

    if args.validation_cores is not None:
        update_config_file_with_comments(
            args.path, args.config, "validation_cores", args.validation_cores
        )

    if args.blast_cores is not None:
        update_config_file_with_comments(
            args.path, args.config, "blast_cores", args.blast_cores
        )

    if args.kraken_db:
        kraken_db = os.path.abspath(args.kraken_db)
        update_config_file_with_comments(
            args.path, args.config, "kraken_db", kraken_db
        )

    if args.kraken_taxonomy:
        update_config_file_with_comments(
            args.path, args.config, "kraken_taxonomy", args.kraken_taxonomy
        )
    ...
    if args.kraken_memory_mapping:
        update_config_file_with_comments(
            ...
            args.kraken_memory_mapping,
        )

    if args.blast_validation is not None:
        update_config_file_with_comments(
            args.path, args.config, "blast_validation", args.blast_validation
        )

    if args.min_perc_identity is not None:
        update_config_file_with_comments(
            args.path, args.config, "min_perc_identity", args.min_perc_identity
        )

    if args.e_val_cutoff is not None:
        update_config_file_with_comments(
            args.path, args.config, "e_val_cutoff", args.e_val_cutoff
        )
    ...
    update_config_file_with_comments(
        args.path, args.config, "main_dir", project_path
    )
