...
# Legacy category lists for backward compatibility
# These are deprecated and will be removed in a future version
# Use get_all_dangerous_pathogens() or get_pathogens_by_threat_level() instead

def _get_legacy_category_list(category: str) -> List[PathogenEntry]:
    """Get pathogens by legacy category name."""
    all_pathogens = get_all_dangerous_pathogens()
    return [p for p in all_pathogens.values() if p.category == category]


# Deprecated: Use get_all_dangerous_pathogens() instead
CDC_CATEGORY_A: List[PathogenEntry] = []
CDC_CATEGORY_B: List[PathogenEntry] = []
CDC_CATEGORY_C: List[PathogenEntry] = []
WHO_PRIORITY_CRITICAL: List[PathogenEntry] = []
FOODBORNE_PATHOGENS: List[PathogenEntry] = []


def _initialize_legacy_lists() -> None:
    """Initialize legacy category lists on first access."""
    global CDC_CATEGORY_A, CDC_CATEGORY_B, CDC_CATEGORY_C
    global WHO_PRIORITY_CRITICAL, FOODBORNE_PATHOGENS

    all_pathogens = list(get_all_dangerous_pathogens().values())

    CDC_CATEGORY_A = [p for p in all_pathogens if p.category == "CDC-A"]
    CDC_CATEGORY_B = [p for p in all_pathogens if p.category == "CDC-B"]
    CDC_CATEGORY_C = [p for p in all_pathogens if p.category == "CDC-C"]
    WHO_PRIORITY_CRITICAL = [p for p in all_pathogens if p.category == "WHO-Critical"]
    FOODBORNE_PATHOGENS = [p for p in all_pathogens if p.category == "Foodborne"]


# Initialize legacy lists when module is imported
try:
    _initialize_legacy_lists()
except Exception:
    # Silently ignore initialization errors (e.g., during testing)
    pass
