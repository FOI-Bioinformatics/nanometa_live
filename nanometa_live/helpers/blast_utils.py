...
def build_blast_databases(
    workdir: str, missing_databases: List[str] = None
) -> NoReturn:
    """
    ...
    Parameters:
        ...
        missing_databases (List[str]): List of missing BLAST databases
        by Tax ID.
    ...
    try:
        ...
        if not os.path.exists(input_folder):
            logging.error(
                f"Input folder {input_folder} does not exist. Exiting."
            )
            return
        ...
        if not files_to_process:
            logging.warning(
                f"No files found in {input_folder}. Nothing to process."
            )
            return

        if not missing_databases:
            logging.info(
                "No missing databases. Skipping BLAST database building."
            )
            return
        ...
        for file in files_to_process:
            ...
            if missing_databases and taxid not in missing_databases:
                logging.info(
                    f"BLAST database already exists for Tax ID: {taxid}. Skipping."
                )
                continue
        ...
        logging.error(
            f"Command failed: {cpe}. Command was {' '.join(cpe.cmd)}."
        )
        raise
...
def check_blast_dbs_exist(
    species_to_taxid: Dict[str, int], data_files_folder: str
) -> List[str]:
    """
    Parameters:
    ...
    if not species_to_taxid:
        logging.warning(
            "Received an empty species to taxid map. No BLAST databases to check."
        )
        return missing_dbs

    if not os.path.exists(data_files_folder):
        logging.error(
            f"Data files folder '{data_files_folder}' does not exist."
        )
        return missing_dbs
    ...
    for species, taxid in species_to_taxid.items():
        ...
        if not os.path.exists(blast_db_file):
            logging.info(
                f"BLAST database for {species} (Tax ID: {taxid}) does not exist."
            )
            missing_dbs.append(str(taxid))
        else:
            logging.info(
                f"BLAST database for {species} (Tax ID: {taxid}) exists."
            )

    if missing_dbs:
        logging.warning(
            f"Missing BLAST databases for Tax IDs: {', '.join(missing_dbs)}"
        )
    else:
