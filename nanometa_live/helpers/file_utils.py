...
def remove_temp_files(config_contents):
    ...
    kraken_results_dir = os.path.join(
        config_contents["main_dir"], "kraken_results/"
    )
    qc_dir = os.path.join(config_contents["main_dir"], "qc_data/")
    qc_file_to_keep = os.path.join(
        config_contents["main_dir"], "qc_data/cumul_qc.txt"
    )
    validation_placeholders = os.path.join(
        config_contents["main_dir"], "validation_fastas/placeholders"
    )
    force_valid_file = os.path.join(
        config_contents["main_dir"], "validation_fastas/force_validation.txt"
    )
    force_blast_file = os.path.join(
        config_contents["main_dir"], "blast_result_files/force_blast.txt"
    )
    fastp_dir = os.path.join(config_contents["main_dir"], "fastp_reports/")
    fastp_file_to_keep = os.path.join(
...
def decompress_zip(zip_filename: str, workingdir: str) -> Union[bool, None]:
    ...
    try:
        logging.info(
            f"Attempting to decompress {zip_filepath} into {workingdir}"
        )
        ...
        logging.info(
            f"Successfully decompressed {zip_filepath} into {workingdir}"
        )
        return True
        ...
        logging.error(
            f"An unexpected error occurred while decompressing {zip_filepath}: {e}"
        )
...
def rename_files(df: pd.DataFrame, workingdir: str):
    try:
        ...
        for subdirectory in subdirectories:
            ...
            species_name = (
                accession  # Default to using the accession as the species name
            )
                    ...
                    for filename in files_in_dir:
                        if filename.endswith(".fna"):
                            source_file = os.path.join(
                                subdirectory_path, filename
                            )
                            target_file = os.path.join(
                                genomes_dir, f"{tax_id}.fasta"
                            )

                            os.rename(source_file, target_file)
                            logging.info(
                                f"Renamed {source_file} to {target_file}"
                            )
                            break

                else:
                    logging.info(
                        f"Accession {accession} is already copied and renamed."
                    )

            else:
                logging.warning(
                    "DataFrame is empty or does not contain 'GID' column. Skipping renaming."
                )
...
def decompress_and_rename_zip(
    zip_filename: str, species_data: Dict[str, Union[str, int]], workingdir: str
) -> bool:
    """
...
def save_species_and_taxid_to_txt(
    df: pd.DataFrame, workdir: str, filename: str = "species_taxid.txt"
):
    """
    ...
    """
    logging.info(
        "Starting the process to save 'Species' and 'Tax_ID' columns to a text file."
    )

    # Check if workdir exists, if not, create it
    if not os.path.exists(workdir):
        logging.info(
            f"Working directory {workdir} does not exist. Creating it."
        )
        os.makedirs(workdir)
    else:
    ...
    logging.info(
        "Found 'Species' and 'Tax_ID' columns in the DataFrame. Proceeding to save."
    )
    ...
    try:
        ...
        logging.info(
            f"Successfully saved 'Species' and 'Tax_ID' columns to {output_path}."
        )
    except Exception as e:
        logging.error(
            f"An error occurred while saving the DataFrame to text: {e}"
        )
        raise
...
def download_genomes_from_ncbi(
    workdir: str,
    prefix: str,
    accession_filename: str = "ncbi_acc_download_list.txt",
):
    """
...
def process_local_files(
    indata_folder: str,
    workdir: str,
    id_dict: Dict[str, Union[str, int]],
    id_type: str = "species",
) -> bool:
    """
    ...
    if missing_files:
        logging.error(
            f"Missing files: {', '.join(missing_files)}. Aborting process."
        )
        return False
    ...
    for original_id, new_id in id_dict.items():
        ...
        logging.info(
            f"Copied and renamed {original_file_path} to {dest_file_path}."
        )
...
def read_and_process_gtdb_metadata(
    file_path: str, kraken_taxonomy: str, species_list: List[str]
) -> pd.DataFrame:
    """
...
def check_genome_files_existence(
    workdir: str, species_taxid_dict: dict
) -> List[str]:
    """
...
def download_from_figshare(figshare_article_id: int, workdir: str) -> None:
    ...
    logging.info(
        f"Initiating Figshare API request for article ID: {figshare_article_id}"
    )
    ...
    logging.info(
        f"Found {len(files_info)} file(s) for article ID: {figshare_article_id}"
    )
    ...
    for file_info in files_info:
        ...
        if os.path.exists(file_path):
            ...
            if file_hash.hexdigest() == checksum:
                logging.info(
                    f"{file_name} already exists and checksum is verified. Skipping download."
                )
                continue
            else:
                logging.info(
                    f"{file_name} exists but checksum is different. Re-downloading."
                )
    ...
    for filename in filenames:
        ...
        # Unzip .gz files that are not .tar.gz
            ...
            if os.path.exists(unzipped_file):
                logging.info(
                    f"{filename} appears to be already unzipped. Skipping."
                )
                continue
            logging.info(f"Unzipping {filename}...")
        ...
        # Unzip .tar.gz files
            ...
            with tarfile.open(file_path, "r:gz") as tar:
                all_files_exist = all(
                    os.path.exists(os.path.join(workdir, member.name))
