...
def read_species_from_config(config_contents: Dict[str, Any]) -> List[str]:
    ...
    if raw_species_list:
        logging.info(
            f"Read {len(raw_species_list)} species from preloaded config."
        )
...
def fetch_species_data(
    search_str: str, db: str, page: int = 1, itemsPerPage: int = 1000
...
def filter_exact_match(rows: list, search_str: str, db: str) -> list:
    ...
    logging.info(
        f"Filtering rows for exact match with search string: {search_str} using database: {db}"
    )
    ...
    filtered_rows = [
        row
        for row in rows
        if row[field].split(";")[-1].strip() == search_str
        and row[rep_field] is True
    ]

    logging.info(
        f"Number of rows after standard filtering: {len(filtered_rows)}"
    )

    if len(filtered_rows) == 0:
        logging.warning(
            f"No rows found that match the search string: {search_str}. Unfiltered rows: {rows}"
        )

    # Additional filtering for NCBI
    if db == "ncbi" and len(filtered_rows) > 1:
        gtdb_rep_rows = [
            row for row in filtered_rows if row["isGtdbSpeciesRep"] is True
        ]

        logging.info(
            f"Number of rows matching 'isGtdbSpeciesRep' after NCBI-specific filtering: {len(gtdb_rep_rows)}"
        )
    ...
    if len(filtered_rows) != 1:
        logging.error("Filtered rows count is not 1. Stopping the program.")
        sys.exit(
            1
        )  # You can replace this with a raise Exception("Filtered rows count is not 1.") if you prefer


def filter_data_by_exact_match(
    data: Dict[str, Dict[str, Any]], db: Any
